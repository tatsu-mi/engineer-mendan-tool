...
function buildTurnInstruction() {
  if (reverseQuestionActive) {
    return `[進行制御]
候補者の直前の発言は逆質問です。最初にその内容を「ご質問は、〜ということですね。」の形で一文に要約してください。質問があれば続けて簡潔かつ誠実に答え、最後に「他にはいかがですか？」と尋ねてください。質問がないことを明確に伝えた場合は、最初に「ご質問は以上ということですね。」と述べ、続けて正確に「面談は以上です。本日はお時間をいただきありがとうございました。後ほど結果をご連絡いたします」と述べてください。`;
  }
...
確定済み主質問${nextQuestionNumber}：${nextQuestion}`
  ...
  if (!canFollowUp) {
    return `[進行制御]
候補者の直前の回答内容を、最初に「ご回答は、〜ということですね。」の形で一文に要約してください。回答にない事実を補わず、固有名詞・数値・担当範囲をできるだけ保持してください。追加の深掘りはせずに次へ進んでください。
${nextAction}`;
  }

  return `[進行制御]
候補者の直前の回答内容を、最初に「ご回答は、〜ということですね。」の形で一文に要約してください。回答にない事実を補わず、固有名詞・数値・担当範囲をできるだけ保持してください。その後、回答が抽象的、判断材料が不足、または重要な経験を具体化できる場合だけ、一度に一つの深掘り質問をしてください。深掘りにはQ番号を付けないでください。
${nextAction}`;
}
...
開始時の挨拶を除き、候補者の回答や逆質問を受けたすべてのターンで、次の質問・説明・終了挨拶へ進む前に、候補者が述べた内容を必ず一文で要約して読み上げてください。この要約は会話ログの評価にも使われるため、単なる相づちにせず、回答に含まれる技術、経験、数値、担当範囲、判断理由などの要点を正確に残してください。聞き取れない内容を推測したり、候補者が述べていない事実を補ったりしないでください。
