...
確定済み主質問${nextQuestionNumber}：${nextQuestion}`
  ...
  if (!canFollowUp) {
    return `[進行制御]
候補者の直前の回答内容を、最初の一文で自然かつ簡潔に復唱してください。「ご回答は、」のような定型的な前置きは使わず、たとえば「JavaでAPI開発を3年間担当され、設計からテストまで経験されたのですね」のように会話として自然につなげてください。回答にない事実を補わず、固有名詞・数値・担当範囲をできるだけ保持してください。追加の深掘りはせずに次へ進んでください。
${nextAction}`;
  }

  return `[進行制御]
候補者の直前の回答内容を、最初の一文で自然かつ簡潔に復唱してください。「ご回答は、」のような定型的な前置きは使わず、たとえば「JavaでAPI開発を3年間担当され、設計からテストまで経験されたのですね」のように会話として自然につなげてください。回答にない事実を補わず、固有名詞・数値・担当範囲をできるだけ保持してください。
...
開始時の挨拶を除き、候補者の回答や逆質問を受けたすべてのターンで、次の質問・説明・終了挨拶へ進む前に、候補者が述べた内容を最初の一文で必ず自然に復唱してください。「ご回答は、」のような定型的な前置きは使わず、内容をそのまま会話につながる文にしてください。この復唱は会話ログの評価にも使われるため、単なる相づちにせず、回答に含まれる技術、経験、数値、担当範囲、判断理由などの要点を正確に残してください。聞き取れない内容を推測したり、候補者が述べていない事実を補ったりしないでください。
