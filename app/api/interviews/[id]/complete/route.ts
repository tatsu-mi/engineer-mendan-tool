...
export async function POST(request: Request, context: RouteContext) {
  ...
  const logs = input.conversationLog;
  const durationSeconds = input.durationSeconds;
  if (
    !Array.isArray(logs)
    || logs.length === 0
    || logs.length > 300
    || !logs.every(isValidLog)
    || logs.reduce((total, log) => total + String(log.text).length, 0) > 2000000
    || !Number.isInteger(durationSeconds)
    || Number(durationSeconds) < 0
  ) {
    return noStoreJson({ error: '会話ログの保存内容が正しくありません。' }, { status: 400 });
  }

  const { error } = await getSupabaseAdmin().rpc('complete_interview', {
  ...
  if (error) {
    return noStoreJson(
      { error: getDatabaseErrorMessage(error, '会話ログを保存できませんでした。') },
      { status: 502 }
    );
  }
