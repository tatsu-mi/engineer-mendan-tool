...
会話ログの候補者発言（画面上の「あなた」）は音声文字起こしであり、誤認識を含む可能性があります。一方、AI面接官は各回答を理解したうえで、次の発言の冒頭で「ご回答は、〜ということですね。」と要約しています。評価では、このAI面接官による回答要約を一次情報として最も重く扱い、候補者発言の文字起こしは文脈確認のための参考情報としてのみ扱ってください。
...
- 候補者の経験・実績・スキルの判定は、候補者発言そのものより、その直後のAI面接官による要約を優先する
- AI面接官の質問、案件説明、逆質問への回答は候補者の実績ではないため、候補者の発言として評価しない
- AI面接官の要約に含まれない候補者発言の情報は参考程度にとどめ、重要な加点・減点の根拠にしない
- AI面接官の要約と候補者発言が食い違う場合は、AI面接官の要約を採用する
